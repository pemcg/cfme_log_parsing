...
def put_timings(o, timings, stage)
  unless timings.nil?
    timings.keys.each do |metric|
      if stage == 'process'
        next if /^num_vim.*/.match(metric)
      end
      next if /total_time/.match(metric.to_s)
  ...
  workers.each do |pid, messages|
    ...
    messages.each do |perf_process|
      ...
      when 'capture_complete'
        ...
        capture_timings = eval(perf_process[:capture_timings]) if @timings_re.match(perf_process[:capture_timings])
        put_timings(o, delta_timings(capture_timings,last_timings), 'capture')
        last_timings = update_timings(last_timings, capture_timings)
      when 'collect_error'
        ...
        put_timings(o, delta_timings(error_timings,last_timings), 'capture')
      ...
      if perf_process[:process_state] == 'process_complete'
        ...
        put_timings(o, delta_timings(process_timings,last_timings), 'process')
...
begin
  ...
  File.foreach( inputfile ) do |line|
    ...
    capture_complete = perf_capture_complete_re.match(line) || storage_capture_complete_re.match(line)
    if capture_complete
      next if workers[capture_complete[:pid]].nil?
      current = workers[capture_complete[:pid]].length - 1 rescue next
      workers[capture_complete[:pid]][current][:capture_state] = 'capture_complete'
      workers[capture_complete[:pid]][current][:capture_timings] = capture_complete[:timings]
      next
    end
